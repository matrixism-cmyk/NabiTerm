...
pub(crate) const CATALOG7: &[(&str, &str, &str, &str)] = &[
    ...
    ("editor.findoddrows", "Find rows with the wrong number of columns", "칸 수가 어긋난 줄 찾기", "列数が合わない行を探す"),
    ("editor.oddrows.none", "Every row has the same number of columns", "모든 줄의 칸 수가 같습니다", "すべての行の列数が同じです"),
    ("editor.oddrows.found", "Rows with a different column count:", "칸 수가 다른 줄:", "列数が異なる行:"),
    ...
    ("sync.movefail", "Could not move:", "옮기지 못함:", "移動できません:"),
    ...
    ("help.cmd.screenshot", "Save a PNG of a pane or the window.", "pane 이나 창을 PNG 로 저장합니다.", "ペインやウィンドウを PNG で保存します。"),
    ...
    ("help.cmd.update", "Upgrade to the latest release, silently.", "최신판으로 조용히 올립니다.", "最新版へ静かに更新します。"),
    ("status.recstart", "Start recording this pane to a file. Click again to stop.", "이 pane 을 파일로 기록하기 시작합니다. 다시 누르면 멈춥니다.", "このペインをファイルに記録し始めます。もう一度押すと停止します。"),
    ("status.tablist", "All open tabs — pick one to jump to it, even if it scrolled off the tab bar.", "열려 있는 탭 전부 — 탭 줄 밖으로 나간 탭도 여기서 고르면 바로 갑니다.", "開いているタブすべて — タブバーの外に出たタブもここから選べます。"),
    ("status.dims.hint", "Pick a size — the window resizes so this pane gets that many columns and rows.", "크기를 고르면 이 pane 이 그만큼의 열·행이 되도록 창 크기를 맞춥니다.", "サイズを選ぶと、このペインがその桁数·行数になるようウィンドウを合わせます。"),
    ("status.clipboard", "Clipboard history (Win+V) — what Windows remembers you copied.", "클립보드 기록(Win+V) — 윈도우가 기억하는 복사한 것들.", "クリップボード履歴(Win+V) — Windows が覚えているコピー内容。"),
    ("clip.empty", "Nothing copied yet.", "아직 복사한 것이 없습니다.", "まだコピーしたものがありません。"),
    ("clip.windows", "Windows clipboard history (Win+V)", "윈도우 클립보드 기록 (Win+V)", "Windows のクリップボード履歴 (Win+V)"),
    ("help.agent.clickcopy", "Click to copy this command", "누르면 이 명령을 복사합니다", "クリックでこのコマンドをコピー"),
    ("help.agent.allcmds", "All commands", "전체 명령", "全コマンド"),
