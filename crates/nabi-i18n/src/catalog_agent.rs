...
pub(crate) const CATALOG_AGENT: &[(&str, &str, &str, &str)] = &[
    ...
    (
        ...
        "pane 안에서 실행 중인 AI가 `nabi cli`로 nabiTerm을 제어할 수 있습니다 — pane 열기·출력 감시·입력 전송. 아래 사용설명을 AI에게 건네세요.",
        "pane 内で動く AI が `nabi cli` で nabiTerm を制御できます — pane を開く・出力監視・入力送信。下のガイドを AI に渡してください。",
    ),
    ("help.agent.examples", "Key commands:", "주요 명령:", "主なコマンド:"),
    ("help.cmd.list", "List all panes (id, title, cwd, state)", "모든 pane 목록(id·제목·cwd·상태)", "全 pane 一覧(id・タイトル・cwd・状態)"),
    ("help.cmd.spawn", "Open a new pane (shell/dir/split)", "새 pane 열기(셸·디렉터리·분할)", "新しい pane を開く(シェル・dir・分割)"),
    ("help.cmd.send", "Send input to a pane (end with \\r = Enter)", "pane에 입력 전송(끝에 \\r = 엔터)", "pane に入力送信(末尾 \\r = Enter)"),
    ("help.cmd.capture", "Read a pane's output (last N lines)", "pane 출력 읽기(최근 N줄)", "pane の出力を読む(直近 N 行)"),
    ...
    ("help.cmd.kill", "Close a pane", "pane 닫기", "pane を閉じる"),
    (
        "help.agent.perm",
        ...
        "권한 모드: 설정 > 동작 > 에이전트 제어(off / ask / on). 'ask'에서 조회 명령은 항상 동작하고, pane의 첫 동작/주입은 승인을 묻습니다.",
    ...
    ("help.agent.hint", "Paste it to the AI in a pane to teach it how to control nabiTerm.", "pane의 AI에게 붙여넣어 nabiTerm 제어법을 알려주세요.", "pane の AI に貼り付けて nabiTerm の制御方法を教えてください。"),
