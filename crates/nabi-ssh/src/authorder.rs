...
//!
//! ## 이 목록이 곧 시도 순서다 (2026-09-10부터)
//!
//! 오래도록 우리는 **고른 키 하나만** 써 보고 끝냈다. 그래서 `~/.ssh` 에 맞는 키가 있어도
//! 세션에 적어 둔 것이 옛것이면 실패했다 — OpenSSH 로는 붙는 서버인데도.
//!
//! 이제 [`crate::authchain`] 이 이 순서대로 실제로 시도한다. 다만 무작정은 아니다:
//! 서버에게 먼저 무엇을 받는지 묻고, 상한(`MAX_ATTEMPTS`)을 넘지 않는다. 넘으면 서버가
//! 끊고, 계정 잠금 정책이 있는 곳에서는 **우리가 사용자 계정을 잠근다.**
//!
//! ## `id_dsa` 는 목록에 없다
//!
...
pub enum Source {
    ...
    Chosen,
    /// `~/.ssh` 에 있는 OpenSSH 기본 이름 — 고른 것이 안 될 때 거들어 보는 것.
    Default,
}
