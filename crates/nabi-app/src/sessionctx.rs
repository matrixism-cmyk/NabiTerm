...
pub(crate) fn session_menu_items(ui: &mut egui::Ui, s: &SavedSession, lang: Lang, folders: &[String]) -> Option<MenuAction> {
    let mut action = None;
    if ui.button(tr(lang, "qc.connect")).clicked() { action = Some(MenuAction::ConnectSaved(s.clone())); ui.close_menu(); }
    if ui.button(tr(lang, "sessions.togglepin")).clicked() { action = Some(MenuAction::TogglePin(s.name.clone())); ui.close_menu(); }
    if ui.button(tr(lang, "sessions.editnote")).clicked() { action = Some(MenuAction::EditNote(s.name.clone())); ui.close_menu(); }
    ...
    if let SessionKind::Ssh { host, port, user, key_path, jump, .. } = &s.kind {
        ...
        if ui.button(tr(lang, "sessions.testconn")).clicked() { action = Some(MenuAction::TestConnection(host.clone(), *port)); ui.close_menu(); }
        if ui.button(tr(lang, "sessions.copyurl")).clicked() {
            if let Some(url) = crate::sshconfig::to_ssh_url(s) { ui.ctx().copy_text(url); }
            ui.close_menu();
        }
    }
    if ui.button(tr(lang, "sessions.edit")).clicked() { action = Some(MenuAction::EditSession(s.clone())); ui.close_menu(); }
    if ui.button(tr(lang, "sessions.duplicate")).clicked() { action = Some(MenuAction::DuplicateSession(s.clone())); ui.close_menu(); }
    // 분할로 연결(MobaXterm식) — 현재 pane 옆/아래에 이 세션을 새 분할로 연다.
    ui.menu_button(tr(lang, "sessions.splitconnect"), |ui| {
        if ui.button(tr(lang, "menu.splitright")).clicked() { action = Some(MenuAction::SplitConnect(s.clone(), true)); ui.close_menu(); }
        if ui.button(tr(lang, "menu.splitdown")).clicked() { action = Some(MenuAction::SplitConnect(s.clone(), false)); ui.close_menu(); }
    });
    ...
    if ui.button(tr(lang, "sessions.delete")).clicked() { action = Some(MenuAction::DeleteSession(s.name.clone())); ui.close_menu(); }
    action
}
