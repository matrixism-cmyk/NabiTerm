...
pub(crate) fn dup_line(text: &str, cursor_char: usize) -> (String, usize) {
...
pub(crate) fn del_line(text: &str, cursor_char: usize) -> (String, usize) {
    let (s, e) = line_bytes(text, cursor_char);
...
pub(crate) fn move_line(text: &str, cursor_char: usize, up: bool) -> (String, usize) {
    let cur = text.char_indices().nth(cursor_char).map(|(b, _)| b).unwrap_or(text.len());
