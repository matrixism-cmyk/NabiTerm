...
pub(crate) fn add_line_numbers(text: &str) -> String {
...
pub(crate) fn keep_unique(text: &str) -> String {
    let lines: Vec<&str> = text.split('\n').collect();
...
pub(crate) fn trim_leading(text: &str) -> String {
    text.split('\n').map(|l| l.trim_start_matches([' ', '\t'])).collect::<Vec<_>>().join("\n")
}

/// 각 줄의 양쪽 공백을 제거한다.
pub(crate) fn trim_both(text: &str) -> String {
    text.split('\n').map(str::trim).collect::<Vec<_>>().join("\n")
}

/// 문서 앞·뒤의 빈 줄(공백만 포함)을 제거한다(중간 빈 줄은 보존).
pub(crate) fn trim_blank_edges(text: &str) -> String {
    let lines: Vec<&str> = text.split('\n').collect();
...
pub(crate) fn reverse_line_chars(text: &str) -> String {
...
pub(crate) fn remove_line_numbers(text: &str) -> String {
