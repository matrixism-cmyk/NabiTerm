...
mod tests {
    ...
    fn local_and_remote_sort_the_same_way() {
        ...
        let rows: Vec<Row> = raw
            .iter()
            .map(|(n, d, sz, mt)| Row {
                name: (*n).into(), is_dir: *d, is_link: false, size: *sz, mtime: *mt, attrs: 0, created: 0,
            })
            .collect();
    ...
    fn folders_stay_first_even_descending() {
        ...
        let mut rows = vec![
            Row { name: "file".into(), is_dir: false, is_link: false, size: 1, mtime: 1, attrs: 0, created: 0 },
            Row { name: "dir".into(), is_dir: true, is_link: false, size: 0, mtime: 2, attrs: 0, created: 0 },
        ];
