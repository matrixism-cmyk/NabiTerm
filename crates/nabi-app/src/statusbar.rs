    ...
    pub(crate) fn status_bar(&mut self, ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::bottom("statusbar").frame(sbar).show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if tg_on { ui.separator(); let (c, sfx) = if tg_err { (crate::theme_ui::ERR, " (!)") } else { (crate::theme_ui::BROADCAST, "") }; ui.colored_label(c, format!("\u{2708} {}{sfx}", tr(lang, "settings.sec.telegram"))).on_hover_text(tr(lang, if tg_err { "tg.r.connerr" } else { "tg.grantall.hint" })); }
