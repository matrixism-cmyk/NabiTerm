...
impl NabiApp {
    ...
    pub(crate) fn persist_view_prefs(&mut self) {
        // 선택 열은 **머리글 오른쪽 클릭으로** 바뀐다(설정 화면이 아니라 목록에서).
        // 그래서 흐름이 반대다 — 패널이 바뀌면 설정으로 저장한다.
        if self.config.terminal.browser_cols != self.browser.cols
            || self.config.terminal.sftp_cols != self.sftp.cols
        {
            self.config.terminal.browser_cols = self.browser.cols.clone();
            self.config.terminal.sftp_cols = self.sftp.cols.clone();
            self.save_config();
        }
