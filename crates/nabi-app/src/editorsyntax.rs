...
pub(crate) fn init(base: &Path, theme: String, ext_map: BTreeMap<String, String>) {
    set_dirs(base);
    set_theme(theme);
    set_ext_map(ext_map);
}
...
pub(crate) fn settings_ui(ui: &mut egui::Ui, e: &mut nabi_config::EditorConfig, lang: nabi_i18n::Lang) {
    use nabi_i18n::tr;
...
pub(crate) fn set_dirs(base: &Path) {
...
pub(crate) fn set_ext_map(map: BTreeMap<String, String>) {
...
pub(crate) fn set_theme(name: String) {
...
/// 현재 테마 이름(없으면 기본).
pub(crate) fn current_theme() -> String {
    THEME
        .get()
...
pub(crate) fn assets() -> &'static RwLock<Assets> {
...
pub(crate) fn mapped_syntax(ext: &str) -> Option<String> {
    EXT_MAP.get()?.read().ok()?.get(ext).cloned()
}

/// 폴더 경로(설정 UI "폴더 열기"용).
pub(crate) fn syntax_dir() -> Option<PathBuf> {
