...
pub(crate) fn extract_urls(t: &str) -> String {
    let mut out = Vec::new();
...
pub(crate) fn extract_emails(t: &str) -> String {
...
pub(crate) fn extract_numbers(t: &str) -> String {
...
pub(crate) fn extract_ips(t: &str) -> String {
...
pub(crate) fn extract_ipv6(t: &str) -> String {
    dedup_keep_order(
...
pub(crate) fn extract_dates(t: &str) -> String {
...
/// 해시태그(#word) 추출.
pub(crate) fn extract_hashtags(t: &str) -> String {
...
pub(crate) fn extract_mentions(t: &str) -> String {
    dedup_keep_order(prefixed(t, '@').into_iter().filter(|w| w[1..].chars().all(|c| c.is_alphanumeric() || c == '_')).collect())
}

/// HEX 색상(#rgb/#rrggbb 등 3·4·6·8자리) 추출.
pub(crate) fn extract_hexcolors(t: &str) -> String {
...
pub(crate) fn extract_macs(t: &str) -> String {
    let is_mac = |s: &str| {
...
pub(crate) fn extract_quoted(t: &str) -> String {
    let mut out = Vec::new();
...
pub(crate) fn extract_menu(ui: &mut egui::Ui, lang: Lang) -> Option<fn(&str) -> String> {
    for (key, f) in [
