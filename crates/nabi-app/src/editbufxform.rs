...
pub(crate) const WHOLE_DOC_LIMIT: usize = 8 * 1024 * 1024;

/// 변환을 적용할 구간을 정한다 — 선택이 있으면 그 범위, 없으면 문서 전체.
///
/// 전체가 한도를 넘으면 `None`(메뉴를 막고 이유를 알린다).
pub(crate) fn target_range(sel: Option<(usize, usize)>, len: usize) -> Option<(usize, usize)> {
    match sel {
...
impl EditBuf {
    ...
    pub(crate) fn apply_transform(&mut self, f: impl Fn(&str) -> String) -> bool {
