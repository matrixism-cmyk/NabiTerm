...
impl eframe::App for NabiApp {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        ...
        self.tick_progress_watch(); // 빌드 진행률을 화면에서 읽는다(배치 AM).
        self.sweep_autolog(); // "모든 세션 기록"이 이름대로 모든 pane 을 보게 한다(배치 AQ).
        self.tick_self_update(); // `nabi cli update` 로 시작한 올리기의 뒷단계.
        self.tick_web_pdf(); // 웹 도구 줄에서 켠 "PDF 로 저장".
        ...
        // 조용히 안 되고 있는 기능들을 **한 번씩만** 알린다(notices 모듈에 규칙과 함께 있다).
        self.tick_notices();
        ...
        self.show_serial_dialog(ctx); // 직렬 콘솔(COM) 열기.
