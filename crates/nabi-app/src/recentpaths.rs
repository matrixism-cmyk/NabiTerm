//! **다녀온 경로를 스스로 기억한다** — 원격도 로컬도 같은 규칙으로.
//!
//! 처음에는 원격(SFTP)에만 두었는데 로컬 브라우저에도 같은 것이 필요해졌다. 규칙을
//! 베끼면 언젠가 한쪽만 고쳐져 "원격에선 스무 개 남는데 로컬에선 열 개만 남는" 일이
//! 생긴다. 그래서 이름을 넓히고(`sftprecent` → `recentpaths`) 두 곳이 이 함수를 부른다.
//!
...
//! 다만 **어디의 경로인지**가 함께 있어야 한다 — `/var/log`는 서버마다 다른 곳이고,
//! 로컬은 그냥 경로다. 그래서 `호스트:경로`(로컬이면 경로만)로 기억한다.

/// 기억할 개수 = **보여 주는 개수**.
///
/// 스물을 기억하고 열만 보여 주고 있었다. 나머지 열은 어디에도 안 나오는데 설정
/// 파일에는 남아 — 사용자에게는 사라진 것이고 디스크에는 남은 셈이었다. 조용한
/// 자름은 없애는 게 낫다: 기억하는 만큼만 기억한다.
