    ...
    fn open_big_viewer(&mut self, path: PathBuf) {
        if let Some(big) = crate::editbig::BigFile::open(&path) {
            ...
            self.add_editor_tab(EditorDoc {
                ...
                minimap: false, wrap_col: 0, rulers: self.editor_config.rulers.clone(), guides: self.editor_config.guides, outline: false, show_lineno: true, bookmarks: Vec::new(), cur_line: 0, syntax_ext: None,
