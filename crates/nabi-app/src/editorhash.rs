...
pub(crate) fn sha1_hex(t: &str) -> String {
    hex(sha1::Sha1::digest(t.as_bytes()).as_slice())
}
pub(crate) fn sha224_hex(t: &str) -> String {
    hex(sha2::Sha224::digest(t.as_bytes()).as_slice())
}
pub(crate) fn sha256_hex(t: &str) -> String {
    hex(sha2::Sha256::digest(t.as_bytes()).as_slice())
}
pub(crate) fn sha384_hex(t: &str) -> String {
    hex(sha2::Sha384::digest(t.as_bytes()).as_slice())
}
pub(crate) fn sha512_hex(t: &str) -> String {
    hex(sha2::Sha512::digest(t.as_bytes()).as_slice())
}

/// Adler-32 체크섬.
pub(crate) fn adler32(t: &str) -> String {
...
/// FNV-1a 32비트.
pub(crate) fn fnv1a32(t: &str) -> String {
...
/// FNV-1a 64비트.
pub(crate) fn fnv1a64(t: &str) -> String {
...
/// djb2 해시(Bernstein).
pub(crate) fn djb2(t: &str) -> String {
    let mut h = 5381u32;
    for &x in t.as_bytes() {
...
/// sdbm 해시.
pub(crate) fn sdbm(t: &str) -> String {
    let mut h = 0u32;
    for &x in t.as_bytes() {
...
pub(crate) fn crc32c(t: &str) -> String {
...
/// CRC-16/CCITT-FALSE.
pub(crate) fn crc16(t: &str) -> String {
...
pub(crate) fn hash_menu(ui: &mut egui::Ui, lang: Lang) -> Option<fn(&str) -> String> {
