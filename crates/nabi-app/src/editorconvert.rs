...
pub(crate) fn json_pretty(t: &str) -> String {
...
pub(crate) fn json_minify(t: &str) -> String {
    serde_json::from_str::<serde_json::Value>(t)
        .ok()
...
pub(crate) fn base64_encode(t: &str) -> String {
...
pub(crate) fn base64_decode(t: &str) -> String {
...
pub(crate) fn url_encode(t: &str) -> String {
...
pub(crate) fn url_decode(t: &str) -> String {
...
pub(crate) fn backslash_escape(t: &str) -> String {
...
pub(crate) fn shell_quote(t: &str) -> String {
    format!("'{}'", t.replace('\'', "'\\''"))
}

/// 역슬래시 이스케이프 해제(\\ \n \t \r \0 \", 그 외 `\c`는 c).
pub(crate) fn backslash_unescape(t: &str) -> String {
    let mut out = String::with_capacity(t.len());
    let mut it = t.chars();
...
/// HTML 특수문자 → 엔티티(& < > " ').
pub(crate) fn html_encode(t: &str) -> String {
    let mut out = String::with_capacity(t.len());
    for c in t.chars() {
        match c {
            '&' => out.push_str("&amp;"),
...
pub(crate) fn html_decode(t: &str) -> String {
    let mut out = String::with_capacity(t.len());
    let mut i = 0;
    while i < t.len() {
        if t.as_bytes()[i] == b'&' {
