...
mod tests {
    ...
    // 시간 측정은 시험에 두지 않는다.
    //
    // 실제로 재 봤다(1만 개, 필터 `*.rs -tmp`). 조용한 회차는 준비 1.7~1.9ms 대 항목마다
    // 3.6~17.5ms 로 **약 2배** 빨랐다. 그런데 같은 코드가 1.7ms 와 16.6ms 를 오갔다 —
    // 이 기계가 빌드로 바쁘면 스케줄링 잡음이 측정을 지배한다.
    //
    // 그래서 시간 단정을 넣으면 **이유 없이 실패하는 시험**이 된다. 그런 시험은 한 번
    // 깨지면 다들 무시하게 되고, 그때부터는 진짜 회귀도 같이 묻힌다. 근거는 숫자가 아니라
    // 하는 일의 양에 둔다 — 준비는 소문자 변환·토큰 나누기를 **한 번**, 항목마다는 **N번**
    // 한다. 위 `prepared_filter_agrees_with_the_shim` 이 둘의 답이 같음을 지킨다.
}
