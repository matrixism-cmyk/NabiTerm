...
pub fn sort_natural(text: &str) -> String {
    sort_by(text, natural_cmp)
}

/// 대소문자 무시 사전식 정렬.
pub fn sort_ci(text: &str) -> String {
    sort_by(text, |a, b| a.to_lowercase().cmp(&b.to_lowercase()))
}

/// 내림차순(Z→A) 사전식 정렬.
pub fn sort_desc(text: &str) -> String {
    sort_by(text, |a, b| b.cmp(a))
}

/// 줄 길이(문자 수) 오름차순 정렬(동률은 사전식).
pub fn sort_by_length(text: &str) -> String {
...
pub fn sort_unique(text: &str) -> String {
    let mut v: Vec<&str> = text.split('\n').collect();
    v.sort_unstable();
    v.dedup();
    v.join("\n")
