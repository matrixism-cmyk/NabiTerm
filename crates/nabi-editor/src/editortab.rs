...
pub fn render_editor_tab(ui: &mut egui::Ui, doc: &mut EditorDoc, lang: Lang, recent: &[String]) -> EditorAct {
    ...
    let dz = crate::uiutil::ctrl_wheel_zoom(ui, over); // Ctrl+휠로 이 에디터만 확대/축소.
...
fn big_view(ui: &mut egui::Ui, doc: &mut EditorDoc, lang: Lang) -> EditorAct {
    ...
    ui.horizontal(|ui| {
        ...
        ui.separator();
        ui.label(format!("{lc} lines \u{00b7} {} \u{00b7} {enc}", crate::humanfmt::human(bytes as u64)));
        if !ready {
            ui.separator();
            ui.spinner();
        }
...
fn editor_body(ui: &mut egui::Ui, doc: &mut EditorDoc, lang: Lang, act: &mut EditorAct) -> (usize, usize, usize) {
    ...
    ui.horizontal_top(|ui| {
        ...
        crate::editortabguide::draw_rulers(ui, doc, out.galley_pos, out.galley.size().y, char_w); crate::editortabguide::draw_guides(ui, doc, &out.galley, out.galley_pos, char_w);
