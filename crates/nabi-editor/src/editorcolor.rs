...
pub fn rgb_to_hsl(t: &str) -> String {
...
pub fn hsl_to_rgb(t: &str) -> String {
...
pub fn rgb_to_grayscale(t: &str) -> String {
    let Some((r, g, b)) = parse_rgb(t) else { return t.to_string() };
...
pub fn rgb_to_cmyk(t: &str) -> String {
    let Some((r, g, b)) = parse_rgb(t) else { return t.to_string() };
    let (rf, gf, bf) = (f64::from(r) / 255.0, f64::from(g) / 255.0, f64::from(b) / 255.0);
...
pub fn cmyk_to_rgb(t: &str) -> String {
...
pub fn hex8_to_rgba(t: &str) -> String {
...
pub fn rgba_to_hex8(t: &str) -> String {
...
pub fn normalize_hex(t: &str) -> String {
    let h = t.trim().trim_start_matches('#');
...
pub fn complementary(t: &str) -> String {
...
pub fn invert_color(t: &str) -> String {
    let h = t.trim().trim_start_matches('#');
    let hex: String = if h.len() == 3 { h.chars().flat_map(|c| [c, c]).collect() } else { h.to_string() };
