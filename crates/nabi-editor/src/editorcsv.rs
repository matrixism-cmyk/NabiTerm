...
pub fn parse_csv(s: &str, delim: char) -> Vec<Vec<String>> {
...
pub fn write_csv(rows: &[Vec<String>], delim: char) -> String {
...
pub fn csv_to_json(t: &str) -> String {
...
pub fn json_to_csv(t: &str) -> String {
...
pub fn csv_to_ndjson(t: &str) -> String {
    let rows = parse_csv(t, ',');
    let Some(headers) = rows.first() else { return t.to_string() };
    rows[1..]
        .iter()
...
pub fn csv_to_html(t: &str) -> String {
    let rows = parse_csv(t, ',');
    if rows.is_empty() {
        return t.to_string();
    }
...
pub fn csv_to_sql(t: &str) -> String {
    let rows = parse_csv(t, ',');
    if rows.len() < 2 {
        return t.to_string();
    }
...
/// CSV 행↔열 전치.
pub fn csv_transpose(t: &str) -> String {
    let rows = parse_csv(t, ',');
    if rows.is_empty() {
        return t.to_string();
    }
...
pub fn csv_sort_rows(t: &str) -> String {
...
pub fn data_menu(ui: &mut egui::Ui, lang: Lang) -> Option<fn(&str) -> String> {
