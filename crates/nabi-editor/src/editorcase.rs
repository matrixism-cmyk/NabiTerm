...
pub fn to_snake(t: &str) -> String {
    per_line(t, |w| w.join("_"))
}
pub fn to_kebab(t: &str) -> String {
    per_line(t, |w| w.join("-"))
}
pub fn to_constant(t: &str) -> String {
    per_line(t, |w| w.iter().map(|x| x.to_uppercase()).collect::<Vec<_>>().join("_"))
}
pub fn to_pascal(t: &str) -> String {
    per_line(t, |w| w.iter().map(|x| cap(x)).collect())
}
pub fn to_camel(t: &str) -> String {
    per_line(t, |w| {
...
pub fn alternating_case(t: &str) -> String {
    let mut n = 0usize;
...
pub fn sentence_case(t: &str) -> String {
    let mut out = String::with_capacity(t.len());
...
pub fn case_menu(ui: &mut egui::Ui, lang: Lang) -> Option<fn(&str) -> String> {
