...
pub type Xf = fn(&str) -> String;

/// (i18n 키, 변환 함수) 목록을 버튼으로 그려 클릭된 함수를 돌려준다. 메뉴 그룹 공용.
pub fn pick(ui: &mut egui::Ui, lang: Lang, items: &[(&str, Xf)]) -> Option<Xf> {
    for (k, f) in items {
...
pub fn sort_menu(ui: &mut egui::Ui, lang: Lang) -> Option<Xf> {
...
pub fn indent_menu(ui: &mut egui::Ui, lang: Lang) -> Option<Xf> {
...
pub fn whitespace_menu(ui: &mut egui::Ui, lang: Lang) -> Option<Xf> {
    use crate::editorlines as l;
    pick(ui, lang, &[
...
pub fn lineops_menu(ui: &mut egui::Ui, lang: Lang) -> Option<Xf> {
    use crate::editorlines as l;
    pick(ui, lang, &[
...
pub fn cipher_menu(ui: &mut egui::Ui, lang: Lang) -> Option<Xf> {
    pick(ui, lang, &[
