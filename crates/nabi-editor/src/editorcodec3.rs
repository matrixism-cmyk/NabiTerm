...
/// 텍스트 → 모스 부호(글자=공백, 단어=` / `).
pub fn morse_encode(t: &str) -> String {
    t.split_whitespace()
...
/// 모스 부호 → 텍스트(`/`=단어 구분).
pub fn morse_decode(t: &str) -> String {
    t.split('/')
...
pub fn a1z26_encode(t: &str) -> String {
    t.chars()
...
pub fn a1z26_decode(t: &str) -> String {
...
pub fn thousands_add(t: &str) -> String {
...
/// 숫자 사이의 천단위 쉼표 제거.
pub fn thousands_remove(t: &str) -> String {
    let ch: Vec<char> = t.chars().collect();
    ch.iter()
        .enumerate()
        .filter(|&(i, &c)| {
