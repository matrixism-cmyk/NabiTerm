...
//!
//! ## 대용량 편집기에는 있는데 여기에 **일부러 없는 것**
//!
//! 쌍둥이 비교(2026-09-07)에서 이 메뉴에 없는 것 셋이 나왔다. 셋 다 옮기지 않는다 —
//! **대칭은 목표가 아니라 단서**이고, 여기에는 옮기지 못할 이유가 있다. 다음 회차가 또
//! 같은 것을 발견하고 또 재 보지 않도록 여기 적어 둔다.
//!
//! * **다중 커서**(`edit.addcursor*`) — 이 창은 egui `TextEdit` 이 글을 그린다. 커서가
//!   하나뿐인 위젯이라 커서를 더 놓을 자리가 없다. 대용량 편집기는 우리가 직접 그리므로
//!   (`EditBuf`) 여러 개를 놓을 수 있다.
//! * **되돌리기/다시 실행**(`editor.undo`/`redo`) — `TextEdit` 이 자기 되돌리기를 갖고
//!   있고 Ctrl+Z / Ctrl+Y 로 동작한다. 밖에서 부를 수 있는 길이 없어 메뉴에 못 건다.
//!   **메뉴에 없는 것이지 기능이 없는 것이 아니다.**
//! * **코드 폴딩**(`fold.*`) — 접으려면 그리는 쪽이 줄을 건너뛸 수 있어야 하는데,
//!   `TextEdit` 은 글 전체를 한 덩이로 그린다.
//!
//! 셋 다 "큰 파일을 여는 창"에서는 된다. 2MB 를 넘으면 그 창으로 열린다.
