...
pub fn dec_to_base36(t: &str) -> String {
    per_line(t, |s| s.parse::<u64>().ok().map(to_base36))
}
pub fn base36_to_dec(t: &str) -> String {
...
pub fn dec_to_base62(t: &str) -> String {
    per_line(t, |s| s.parse::<u64>().ok().map(to_base62))
}
pub fn base62_to_dec(t: &str) -> String {
...
pub fn sec_to_hms(t: &str) -> String {
    per_line(t, |s| {
...
/// `H:MM:SS`/`M:SS`/`S` → 초.
pub fn hms_to_sec(t: &str) -> String {
    per_line(t, |s| {
        let mut total: u64 = 0;
        for part in s.split(':') {
...
pub fn bytes_human(t: &str) -> String {
    per_line(t, |s| s.parse::<u64>().ok().map(human))
}
pub fn human_bytes(t: &str) -> String {
    per_line(t, |s| parse_human(s).map(|n| n.to_string()))
}

/// "숫자" 서브메뉴 — 클릭한 변환 함수를 돌려준다.
pub fn num_menu(ui: &mut egui::Ui, lang: Lang) -> Option<fn(&str) -> String> {
