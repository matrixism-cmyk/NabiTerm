...
pub fn quote_lines(t: &str) -> String {
    t.split('\n').map(|l| format!("> {l}")).collect::<Vec<_>>().join("\n")
}

/// 각 줄 앞의 인용부호(`> ` 또는 `>`)를 제거.
pub fn unquote_lines(t: &str) -> String {
    t.split('\n')
...
pub fn md_ordered(t: &str) -> String {
    let mut n = 0;
    t.split('\n')
        .map(|l| {
            if l.trim().is_empty() {
...
pub fn md_bullet(t: &str) -> String {
    map_nonempty(t, |l| format!("- {l}"))
}

/// 각 줄 앞에 우측정렬 줄 번호 + 탭(`nl` 명령식, 빈 줄 포함 — md 순서목록과 구별).
pub fn number_lines(t: &str) -> String {
...
pub fn list_menu(ui: &mut egui::Ui, lang: Lang) -> Option<fn(&str) -> String> {
