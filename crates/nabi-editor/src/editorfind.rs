...
pub fn compiled(f: &FindState) -> Option<regex::Regex> {
...
impl FindState {
    ...
    pub fn search_only(&self) -> FindState {
        FindState {
...
pub fn needs_regex(f: &FindState) -> bool {
    f.regex || f.whole
}

/// 찾기 바를 그린다(에디터 본문 위). 일치 줄로 scroll_to를 설정한다.
pub fn find_bar(ui: &mut egui::Ui, doc: &mut EditorDoc, lang: Lang) {
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        if g.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter)) {
            if let Some((line0, col)) = crate::editorloc::parse_goto(&doc.find.goto) {
                ...
                if let Some(c) = col { doc.find.pending_cursor = Some(crate::textpos::line_col_to_offset(&doc.text, line0, c)); }
            }
        }
    });
}
