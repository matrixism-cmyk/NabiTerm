...
pub fn paint(
    ...
    for (r, row) in rows_v.iter().enumerate() {
        ...
        let mut hl: Vec<Option<Rgba>> = match find {
            Some(q) if !q.is_empty() => crate::findhl::match_cells(row, q)
                .iter()
                .map(|&b| b.then_some(theme.match_color))
                .collect(),
            _ => Vec::new(),
        };
        if !rules.is_empty() {
            ...
            for (word, color) in &rules {
                for (i, m) in crate::findhl::match_cells(row, word).iter().enumerate() {
        ...
        paint_row(
            &painter, rect, r as f32, cw, ch, &font, row, theme, &hl, ul, sel, rect_sel,
        );
    }
